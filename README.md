...
本框架**并不**直接与qq等应用桥接，而是需要一个**OneBot实现端**作为中间层来与qq之间进行通信，换言之，本框架实际上仅为一个**后端框架**
...
* 独特的消息模式匹配器
  * 框架内置了一个基于NFA和带权BFS的消息匹配器，用于在丢弃过时的CQ码后适配较新的数组消息格式，支持构建包含循环节点（组）、可选节点（组）与组嵌套的复杂匹配模式，并能在匹配过程中自然实现贪婪语义，无需采用传统正则引擎回溯匹配方式，性能更优
  * 消息模式匹配器以Builder模式使用流式方法调用构建，使用方便

* 支持复杂依赖关系的插件系统
  * 插件加载器基于依赖图设计，使用拓扑排序自动检测循环依赖与缺失依赖，并确保插件按依赖顺序加载
  * 当某个插件加载失败时，所有依赖它的插件也会被标记为失败，避免在运行时出现更严重的错误
  * 未来计划引入插件版本管理机制，使其拥有接近包管理器的精确度

* 基于MethodHandle的事件总线设计
  * 框架内置了一个同步事件总线，基于注解自动对订阅者进行发现与注册
  * 总线使用MethodHandle进行性能优化，相比部分使用反射调用的事件总线性能更优
  
* 开箱即用的使用体验
  * 框架在内部拥有完整的网络层实现与启动流程，可在完全空载的情况下直接启动
  * 导入框架后仅需写一个简单的命令执行器便可开始实现功能
  * 框架内置多种实用工具，为开发者的使用带来便利

* 基于现实项目的优化迭代
  * 框架开发者本人同时维护着一个基于此框架的实际bot项目，通过在项目开发时的痛点对框架进行优化，在一定程度上保证了实际开发时的使用体验
...
框架本体内置了以下功能：
 ...
 - 事件总线（使用注解进行注册）
 - bot账号信息及群聊/好友上下文管理器
 - 基于JLine3的交互式控制台（支持扩展指令）
 - 支持有复杂依赖关系的插件加载器（为应用带来更多可能）
 - 一个使用NFA匹配消息段的消息模式匹配器DSL（为复杂的数组消息匹配设计）
