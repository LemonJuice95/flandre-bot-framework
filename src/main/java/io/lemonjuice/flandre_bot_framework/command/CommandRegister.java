...
public class CommandRegister {
    ...
    public void load() {
        this.commands.forEach((c) -> {
            getDestination(c.apply(Message.DUMMY).getType()).add(c);
        });
    }

    private static List<Function<Message, CommandRunner>> getDestination(CommandRunner.Type type) {
        return switch (type) {
            case GROUP -> BotCommandLookup.GROUP_COMMANDS;
            case PRIVATE -> BotCommandLookup.PRIVATE_COMMANDS;
            case FRIEND -> BotCommandLookup.FRIEND_COMMANDS;
